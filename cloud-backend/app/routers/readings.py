...
@router.post("/readings/manual", response_model=schemas.ReadingResponse, status_code=201)
def create_manual_reading(reading_in: schemas.ManualReadingCreate, db: Session = Depends(get_db)):
    """A human-entered glucose reading (e.g. from a glucometer) — no sensor, no
    calibration coefficient required. Written into the same GlucoseReading table
    as sensor readings (source="manual") so it shows up on the same trend chart
    and feeds the same hypo-drop velocity/alert engine, which only depends on
    estimated_glucose_mg_dl + timestamp, not on how the reading was produced."""
    dog = crud.get_dog(db, reading_in.dog_id)
    if dog is None:
        raise HTTPException(status_code=404, detail="Dog not found")
    ...
    velocity_str = f"{velocity:+.2f}" if velocity is not None else "n/a"
    logger.info(
        "Dog BG (manual): %.1f mg/dL (Δ %s mg/dL/min) [dog_id=%s]",
        reading.estimated_glucose_mg_dl,
        velocity_str,
        reading.dog_id,
    )
